...
function get_dissipation_fields(t::VarianceDissipation, tracer_name) 
    ...
    diffusive_prod = isnothing(D) ? nothing : Tuple(getproperty(D, dir) for dir in dirs)

    if isnothing(diffusive_prod)
        return NamedTuple{tuple(prod_names..., grad_names...)}(tuple(advective_prod..., grad...))
    else
